...
function connectWebSocket() {
    ...
    ws.onmessage = (event) => {
        ...
        // 🔄 ሪል-ታይም የካርዶች ሁኔታ ማሻሻያ
        if (data.type === "taken_cards_update" || data.taken_cards) {
            update200CardsColors(data.taken_cards || []);
        }

        // 1️⃣ 🛠 ፊክስ፦ ከባክኤንዱ የ "countdown" መልእክት ጋር እንዲስማማ ማድረግ
        if ((data.type === "countdown" || data.type === "time_update") && data.phase === "PICK") {
            ...
            if(timerText) {
                // 'seconds' ወይም 'time' ካለ በሱ እንዲቆጥር ማድረግ
