...
class GameEngine:
        ...
        while self.running:
            ...
            try:
                ...
                if self.running and has_bought_cards:
                    ...
                    # ጨዋታው ካርድ ሳይሸጥበት ካለቀ ሁኔታውን 'cancelled' እናደርገዋለን
                    try:
    ...
    async def countdown(self, seconds, game_display_no, saved_game_id):
        ...
        while seconds >= 0 and self.running:
            ...
            try:
                ...
                # 📊 የተሸጡ እውነተኛ ካርዶችን ቁጥር መቁጠር (የማታለያ ኮድ የለም!)
            ...
            # 💰 የደራሽ (Derash) ስሌት፡ ጠቅላላ እውነተኛ ሽያጭ * 0.80 (20% ተቀንሶ)
            game_fee = 10.0
            ...
            payload = {
                ...
                "player_count": sold_cards_count  # እውነተኛ የተሸጡ ካርዶች ብቻ
            }
            await self.safe_broadcast(payload)
            print(f"⏱ server countdown tick: {seconds} (game {game_display_no}) | Sold: {sold_cards_count} | Derash: {derash_amount}")
    ...
    async def draw_numbers(self, interval, game_display_no, saved_game_id):
        ...
        try:
            ...
            # 👥 የሙከራ ባይፓስ (Test Mode) ኮድ ሙሉ በሙሉ ተወግዷል! እውነተኛ መረጃ ብቻ።
            ...
            for number in numbers:
                ...
                # አሸናፊ መኖሩን መፈተሽ (የተሸጡትንም ያልተሸጡትንም)
...
BOT_NAMES = [
    ...
    def process_drawn_ball_and_check_winner(self, db, game_id, current_drawn_balls, total_pool_money, bought_cards, all_200_cards):
        # 1. መጀመሪያ እውነተኛ ተጫዋቾች መካከለ አሸናፊ መኖሩን መፈተሽ
        ...
        # 2. እውነተኛ ተጫዋች ካላሸነፈ፣ ያልተሸጡት (የአድሚን) ካርዶች ቢንጎ ማለታቸውን መፈተሽ
    ...
    def distribute_game_prize(self, db, game_id, total_pool_money, winner_user_id=None, winning_card=None):
        ...
        # 💰 የገንዘብ ክፍፍል ሕግ
        if winner_user_id:
            # እውነተኛ ተጫዋች ካሸነፈ 20% ኮሚሽን ለአድሚን ይገባል፣ 80% ለተጫዋቹ ሚዛን
            ...
            # 🏛 የአድሚን ካርድ ካሸነፈ 100% ሙሉው ገንዘብ (የተጫዋቾች መወራረጃ) ወደ አድሚን 'house_balance' ይገባል!
