...
def send_welcome(message):
    ...
    # የፎቶ አድራሻ (ካለህ የሎጎውን ሊንክ እዚህ መተካት ትችላለህ ወይም በ send_message መቀየር ትችላለህ)
...
def menu_check_balance(message):
    ...
    try:
        # ከ FastAPI ተጠቃሚውን መፈለግ
        response = requests.get(f"{BACKEND_URL}/api/users/{telegram_id}") # እንደ ባክአንድህ ኤፒአይ አወቃቀር ይወሰናል
...
def process_deposit_amount(message):
    ...
    # መጠን ከተቀበለ በኋላ የ Manual-Payment ቁልፍ ማሳየት (ወደ ሚኒ አፕ የሚወስድ)
    markup = types.InlineKeyboardMarkup()
    # የገባውን መጠን በ query parameter መልክ ወደ ሚኒ አፑ እናስተላልፋለን
...
def command_withdraw(message):
    ...
    # ከተጫዋቹ ባክአንድ ላይ የባላንስ መረጃ ለመጠየቅ (ኤፒአይ መጥራት ይቻላል)
    # ለጊዜው ቀጥታ ወደ ዊዝድሮው ሚኒ አፕ ገጽ ይመራዋል
    markup = types.InlineKeyboardMarkup()
...
def handle_admin_actions(call):
    ...
    target_id = action_data[2]  # ID ቁጥር

    if tx_type == "dep":
        url = f"{BACKEND_URL}/api/deposit/admin/approve"
        payload = {"deposit_id": int(target_id), "action": "APPROVE" if action == "app" else "REJECT", "admin_telegram_id": str(ADMIN_TELEGRAM_ID)}
    else:
        url = f"{BACKEND_URL}/api/withdraw/admin/approve"
        payload = {"withdraw_id": int(target_id), "action": "APPROVE" if action == "app" else "REJECT", "admin_telegram_id": str(ADMIN_TELEGRAM_ID)}
