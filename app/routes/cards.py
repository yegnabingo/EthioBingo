...
def pick_card(request: AdvancedPickCardRequest):
    ...
    try:
        ...
        # 8. ካርዱን ለተጫዋቹ መመዝገብ (የተመረጠውን Bet መጠን አካትቶ መያዝ)
        new_player_card = PlayerCard(
            game_id=game.id,
            user_id=user.id,
            card_number=request.card_number,
            bet_amount=request.bet_amount  # 🎯 [ዋናው ማስተካከያ] የሰበሰበውን ውርርድ እዚህ ይመዘግባል!
        )
        
        # የጌሙን ጠቅላላ ገቢ (Pool) እና የተጫዋቾች ብዛት በዳታቤዝ ላይ በቅጽበት ማሳደግ
